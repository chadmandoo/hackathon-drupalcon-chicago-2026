...
Usage:
    python check_submission.py /path/to/submission
"""
...
def main() -> int:
    ...
    parser.add_argument("path", help="Path to the submission folder")
    ...
    has_skill_or_runbook = any(
        ("skill" in f.name.lower() or "runbook" in f.name.lower())
        for f in files
    )
    has_validator = any(
        ("validator" in f.name.lower() or "check_" in f.name.lower())
        for f in files
    )
    ...
    # Heuristic check for "real agent work moment"
